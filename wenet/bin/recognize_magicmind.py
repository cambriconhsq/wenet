...
from magicmind.python.runtime import Network, BuilderConfig, Builder, Device, System, Model, Builder
...
def main():
    ...
    # encoder
    encoder_model = Model()
    encoder_model.deserialize_from_file(args.encoder_magicmind)
    encoder_engine = encoder_model.create_i_engine()
    encoder_context = encoder_engine.create_i_context()
    inputs = encoder_context.create_inputs()
    encoder_outputs = []

    # decoder
    decoder_model = Model()
    decoder_model.deserialize_from_file(args.decoder_magicmind)
    decoder_engine = decoder_model.create_i_engine()
    decoder_context = decoder_engine.create_i_context()
    decoder_inputs = decoder_context.create_inputs()
    decoder_outputs = []
    ...
    with torch.no_grad(), open(args.result_file, 'w') as fout:
        for _, batch in enumerate(test_data_loader):
            ...
            inputs[0].from_numpy(feats) 
            inputs[1].from_numpy(feats_lengths)
            assert encoder_context.enqueue(inputs, encoder_outputs, queue).ok()
            ...
            encoder_out = encoder_out.asnumpy()
            encoder_out_lens = encoder_out_lens.asnumpy()
            ctc_log_probs = ctc_log_probs.asnumpy()
            beam_log_probs = beam_log_probs.asnumpy()
            beam_log_probs_idx = beam_log_probs_idx.asnumpy()
            ...
            if args.mode == 'attention_rescoring':
                ...
                for i in range(batch_size):
                    for j in range(beam_size):
                        ...
                        k += 1
                decoder_inputs[0].from_numpy(encoder_out)
                decoder_inputs[1].from_numpy(encoder_out_lens)
                decoder_inputs[2].from_numpy(hyps_pad_sos_eos)
                decoder_inputs[3].from_numpy(hyps_lens_sos)
                decoder_inputs[4].from_numpy(ctc_score)

                decoder_context.enqueue(decoder_inputs, decoder_outputs, queue)
                queue.sync()
                best_index = decoder_outputs[0].asnumpy()
